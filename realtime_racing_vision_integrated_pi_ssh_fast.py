...
# Make OpenCV GUI windows behave better over SSH/X11 forwarding.
# This must be set before importing cv2 on many Linux/OpenCV builds.
...
# GPS
# ----------------------------
# Raspberry Pi / Linux serial ports are usually one of:
#   /dev/serial0  -> Pi GPIO UART
#   /dev/ttyUSB0  -> USB GPS dongle
#   /dev/ttyACM0  -> some USB GPS modules
# You can override without editing the file:
#   GPS_PORT=/dev/ttyUSB0 python3 realtime_racing_vision_integrated_pi.py
PORT = os.environ.get("GPS_PORT", "/dev/ttyACM0")
...
# Raspberry Pi / Linux camera settings.
# Override without editing, for example:
#   CAMERA_INDEX=2 CAMERA_WIDTH=1280 CAMERA_HEIGHT=720 python3 realtime_racing_vision_integrated_pi.py
...
# If your stereo camera appears as one side-by-side wide frame, keep this True.
# Set STEREO_SPLIT=0 to process the full camera frame instead.
STEREO_SPLIT = os.environ.get("STEREO_SPLIT", "1") != "0"
STEREO_SIDE = os.environ.get("STEREO_SIDE", "left").lower()  # left or right
...
# ----------------------------
# SSH / remote window display
# ----------------------------
# To see OpenCV windows over SSH, start SSH with X11 forwarding from your laptop:
#   ssh -X pi@raspberrypi.local
# or, if -X fails:
#   ssh -Y pi@raspberrypi.local
#
# Then run this script normally. If DISPLAY is set, OpenCV windows will be opened
# on your SSH client computer. If DISPLAY is not set, the script will keep running
# but will disable OpenCV windows to avoid crashing.
ENABLE_OPENCV_WINDOWS = os.environ.get("ENABLE_OPENCV_WINDOWS", "1") != "0"
REQUIRE_OPENCV_WINDOWS = os.environ.get("REQUIRE_OPENCV_WINDOWS", "0") == "1"

# Fast SSH debug display settings. X11 forwarding is slow, so do not send
# full 1280x720 frames every loop. Edge detection still runs on the full frame;
# only the debug preview windows are resized/throttled.
DEBUG_DISPLAY_SCALE = float(os.environ.get("DEBUG_DISPLAY_SCALE", "0.35"))
DEBUG_DISPLAY_EVERY_N = max(1, int(os.environ.get("DEBUG_DISPLAY_EVERY_N", "5")))
DEBUG_EDGE_WINDOW_W = int(os.environ.get("DEBUG_EDGE_WINDOW_W", "640"))
DEBUG_EDGE_WINDOW_H = int(os.environ.get("DEBUG_EDGE_WINDOW_H", "360"))

def make_preview(frame, scale=DEBUG_DISPLAY_SCALE):
    if frame is None:
        return frame
    if scale >= 0.99:
        return frame
...
# SHOW_HUD now means OLED HUD output, not a PC OpenCV HUD window.
# The edge video, GPS map, and track-position debug windows are unchanged.
DISPLAY_W = 1280
DISPLAY_H = 960
START_DIST_M = 100.0
...
OLED_UPDATE_EVERY = HUD_UPDATE_EVERY
...
track_position_lock = threading.Lock()
current_track_position = "WAITING"
...
def compute_brake_marker(local_turn, track_position):
    """
    Returns the one-letter braking hint shown next to distance on the HUD.

    L = outside line for the upcoming corner, brake earlier / late-braking warning
    E = inside line for the upcoming corner, early-braking opportunity
    N = normal / centered / no reliable track-position estimate
    """
    if local_turn not in ("LEFT", "RIGHT"):
        return "N"

    if track_position == "CENTER" or track_position == "WAITING":
        return "N"

    # User-requested example: car LEFT + upcoming RIGHT turn = outer line = L.
    if local_turn == "RIGHT":
        if track_position == "LEFT":
            return "L"
        if track_position == "RIGHT":
            return "E"

    # Inverse case for left-hand corners.
    if local_turn == "LEFT":
        if track_position == "RIGHT":
            return "L"
        if track_position == "LEFT":
...
def set_current_track_position(position):
    global current_track_position
    with track_position_lock:
        current_track_position = position


def get_current_track_position():
    with track_position_lock:
        return current_track_position
...
class OLEDHUD:
    def __init__(self):
        if not OLED_AVAILABLE:
            raise RuntimeError(f"OLED import failed: {OLED_IMPORT_ERROR}")
    ...
    def show_turn(self, direction, distance_m, brake_marker):
        ...
        # Same OLED HUD layout as your simulator, with the braking marker added
        # next to the GPS distance. Example: "75 m  L".
        self._text_center(draw, f"{max(distance_m, 0):.0f} m  {brake_marker}", self.height - 18)
    ...
    def update_from_state(self):
        ...
        track_position = get_current_track_position()
        brake_marker = compute_brake_marker(local_turn, track_position)

        if local_active and local_turn:
            self.show_turn(local_turn, local_dist, brake_marker)
        else:
            self.show_clear()
...
def render_hud(frame):
    ...
    local_track_position = get_current_track_position()
    local_brake_marker = compute_brake_marker(local_turn, local_track_position)
    ...
    if local_active and local_turn:
        ...
        dist_text = f"{max(local_dist, 0):.0f} m  {local_brake_marker}"
...
def find_linux_gps_port():
    """Return a likely Linux GPS serial port, or None if no candidate exists."""
    candidates = []
    for pattern in ("/dev/serial0", "/dev/ttyUSB*", "/dev/ttyACM*", "/dev/ttyAMA*"):
...
def open_serial():
    port = PORT
    if port == "auto":
        port = find_linux_gps_port()
        if port is None:
            raise RuntimeError("No GPS serial port found. Try GPS_PORT=/dev/ttyUSB0 or GPS_PORT=/dev/serial0")
...
def gps_reader_thread():
    ...
    track_model.update_status(f"GPS opened @ {BAUD}")
...
def video_reader_thread():
    ...
    # Raspberry Pi/Linux: force the V4L2 backend so OpenCV does not fall back
    # to a laptop/webcam backend or Windows-only DirectShow behavior.
    cap = cv2.VideoCapture(CAMERA_INDEX, cv2.CAP_V4L2)

    # Stereo camera often outputs one wide side-by-side frame.
    # Default: 2560x720 = 1280x720 left + 1280x720 right.
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, CAMERA_WIDTH)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, CAMERA_HEIGHT)
    cap.set(cv2.CAP_PROP_FPS, CAMERA_FPS)

    # Request MJPG when supported; this usually improves USB camera FPS on Pi/Linux.
    try:
    ...
    print(f"Camera opened successfully on /dev/video{CAMERA_INDEX}.")
    print(f"Requested camera mode: {CAMERA_WIDTH}x{CAMERA_HEIGHT} @ {CAMERA_FPS} FPS")
    if STEREO_SPLIT:
        print(f"Using {STEREO_SIDE.upper()} stereo half for edge detection.")
        video_width = CAMERA_WIDTH // 2
    else:
        print("Using full camera frame for edge detection.")
    ...
    while video_running:
        ...
        if not ret:
            print("Failed to grab frame from stereo camera.")
            time.sleep(0.01)
            continue
        ...
        if STEREO_SPLIT:
            mid = w // 2
            if STEREO_SIDE == "right":
                selected_frame = frame[:, mid:]
            else:
                selected_frame = frame[:, :mid]
        else:
        ...
        # This keeps the rest of the edge detection, HUD, and position code unchanged.
        with frame_lock:
            frame_queue.clear()
            frame_queue.append(selected_frame.copy())

    cap.release()


# ============================================================
# SSH / GUI WINDOW HELPERS
# ============================================================

def running_over_ssh():
    return bool(os.environ.get("SSH_CONNECTION") or os.environ.get("SSH_CLIENT"))


def opencv_gui_available():
    """Return True when OpenCV windows can be created safely."""
    if not ENABLE_OPENCV_WINDOWS:
        return False

    # On Linux, OpenCV HighGUI needs DISPLAY for X11/Wayland GUI windows.
    # Over SSH this appears only when you connect with ssh -X or ssh -Y.
...
def print_gui_startup_status(gui_enabled):
    if gui_enabled:
        print("OpenCV GUI windows enabled.")
        print(f"DISPLAY={os.environ.get('DISPLAY', '')}")
        if running_over_ssh():
            print("SSH detected: windows should appear on the computer you SSH'd from.")
        return

    print("OpenCV GUI windows disabled because no graphical DISPLAY was found.")
    if running_over_ssh():
        print("To show windows over SSH, disconnect and reconnect using:")
        print("  ssh -X pi@raspberrypi.local")
        print("or:")
        print("  ssh -Y pi@raspberrypi.local")
        print("Then confirm with: echo $DISPLAY")
    else:
        print("If using the Pi desktop/HDMI, make sure you run from a terminal inside the desktop session.")

    if REQUIRE_OPENCV_WINDOWS:
        raise RuntimeError("OpenCV windows were required, but DISPLAY is not available.")
...
def main():
    ...
    oled_hud = None
    gui_enabled = opencv_gui_available()
    print_gui_startup_status(gui_enabled)

    local_show_original = SHOW_ORIGINAL_VIDEO and gui_enabled
    local_show_edge = SHOW_EDGE_VIDEO and gui_enabled
    local_show_gps_map = SHOW_GPS_MAP and gui_enabled
    local_show_track_position = SHOW_TRACK_POSITION and gui_enabled
    any_opencv_window = local_show_original or local_show_edge or local_show_gps_map or local_show_track_position
    ...
    if local_show_original:
        cv2.namedWindow("Original Video", cv2.WINDOW_NORMAL)

    if local_show_edge:
        cv2.namedWindow("Edge Detected Video", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("Edge Detected Video", DEBUG_EDGE_WINDOW_W, DEBUG_EDGE_WINDOW_H)

    if SHOW_HUD:
        try:
            oled_hud = OLEDHUD()
            print("OLED HUD opened successfully.")
        except Exception as e:
            oled_hud = None
            print(f"ERROR: Could not open OLED HUD: {e}")
            print("Continuing without OLED HUD. Edge video and track-position windows still run.")

    if local_show_gps_map:
        cv2.namedWindow("GPS Map", cv2.WINDOW_NORMAL)

    if local_show_track_position:
        cv2.namedWindow("Track Position", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("Track Position", POSITION_DISPLAY_W, POSITION_DISPLAY_H)

    # Kept for optional/future PC HUD rendering, but SHOW_HUD now writes to OLED.
    ...
    print(f"Fast SSH preview: scale={DEBUG_DISPLAY_SCALE}, show every {DEBUG_DISPLAY_EVERY_N} processed frames")
    ...
    try:
        while True:
            ...
            if frame is not None:
                ...
                show_debug_frame = (loop_count % DEBUG_DISPLAY_EVERY_N == 0)

                if local_show_original and show_debug_frame:
                    cv2.imshow("Original Video", make_preview(frame))

                if local_show_edge and show_debug_frame:
                    cv2.imshow("Edge Detected Video", make_preview(edge_frame))

                if local_show_track_position:
                    position_estimator.update(scored_edges)
                    set_current_track_position(position_estimator.stable_position)
                ...
                if SHOW_HUD and oled_hud is not None and (loop_count % OLED_UPDATE_EVERY == 0):
                    oled_hud.update_from_state()

                if local_show_gps_map and show_debug_frame:
                    if gps_map is None or loop_count % GPS_MAP_UPDATE_EVERY == 0:
                        gps_map = render_gps_map()
                    cv2.imshow("GPS Map", make_preview(gps_map))
                ...
                # X11 is slow, so only pump the GUI when we actually update a preview.
                key = (cv2.waitKey(1) & 0xFF) if (any_opencv_window and show_debug_frame) else 255
            else:
                ...
                key = (cv2.waitKey(1) & 0xFF) if any_opencv_window else 255
        ...
        if oled_hud is not None:
            oled_hud.close()

        cv2.destroyAllWindows() if any_opencv_window else None
