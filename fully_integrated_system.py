...
GPS_DEBUG = os.environ.get("GPS_DEBUG", "0") == "1"
R_EARTH = 6371000.0
...
def open_serial():
    # This matches the working GPS code style: open exactly one serial port
    # and continuously read NMEA lines from it.
    return serial.Serial(PORT, BAUD, timeout=1)


def parse_gps_fix(raw):
    """Parse one NMEA line. Returns fix dict or None.

    The original working code used GGA only. This keeps GGA as the main path,
    but also accepts RMC as a backup because some modules output valid RMC
    more often than valid GGA.
    """
    ...
    try:
        ...
        if GPS_DEBUG:
            print(f"GPS parse skipped: {e}")
        return None

    # Primary path: same as your working code.
    if isinstance(msg, pynmea2.GGA):
        ...
        if GPS_DEBUG:
            print(f"GPS GGA: quality={fix_quality}, lat={msg.latitude}, lon={msg.longitude}, sats={msg.num_sats}, hdop={msg.horizontal_dil}")
    ...
    # Backup path: accept RMC/GNRMC valid fixes too.
    if isinstance(msg, pynmea2.RMC):
        status = getattr(msg, "status", "")
        if GPS_DEBUG:
            print(f"GPS RMC: status={status}, lat={msg.latitude}, lon={msg.longitude}")
...
def gps_reader_thread():
    try:
        ...
        print(f"GPS serial open failed on {PORT} @ {BAUD}: {e}")
        return

    track_model.update_status(f"GPS opened on {PORT} @ {BAUD}")
    print(f"GPS opened on {PORT} @ {BAUD}")
